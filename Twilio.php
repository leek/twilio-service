...
class Twilio extends Zend_Rest_Client
{
     ...
     */
    public function sendSms($from, $to, $body, array $options = array())
    {
        ...
        // Check Caller Number
        $len = iconv_strlen($from, 'UTF-8');
        if ($len < 10) {
            require_once 'Twilio/Exception.php';
            throw new Twilio_Exception('From Number must be at least 10 characters (eg. XXX-XXX-XXXX)');
        }

        // Check Called Number
        $len = iconv_strlen($to, 'UTF-8');
        if ($len < 10) {
            require_once 'Twilio/Exception.php';
            throw new Twilio_Exception('To Number must be at least 10 characters (eg. XXX-XXX-XXXX)');
        }
        ...
        $this->_init();
        $path = sprintf('/%s/Accounts/%s/SMS/Messages',
            $this->getApiVersion(),
            $this->getAccountSid()
        );
     ...
     * @return Twilio_Result
     */
    public function makeCall($from, $to, $url, array $options = array())
    {
        // Will hold what we send to Twilio
        $data = array();
                    ...
                    switch ($option) {
                        ...
                        case 'Method':
                            ...
                            if (!in_array($value, array('POST', 'GET'))) {
                                require_once 'Twilio/Exception.php';
                                throw new Twilio_Exception($option . ' value must be one of POST or GET');
                            }
                            break;
                        case 'SendDigits':
